############################################################################################
############################################################################################
### Multiple imputation script: time to treatment intensification
############################################################################################
############################################################################################
...
########################################
### Section 4: Pool imputed models and extract pooled estimates (PO & PH) for intensification outcome
############################################################################################
...
mi_data <- mi_data %>% select(-ins_ever, -time_insulin_yrs)
...
mi_data$H0 <- nelsonaalen(mi_data, timevar = time_change_yrs, statusvar = intens_event)
# setup run
imp0 <- mice(mi_data, maxit = 0)
pred <- imp0$predictorMatrix
# remove event time from predictor (high correlation with H0)
pred[, "time_change_yrs"] <- 0

############################################################################################
### Section 3: Perform multiple imputation and fit Royston–Parmar models (loop over 30 imputations)
############################################################################################
# This loop performs 30 independent imputations (m=1 each), saves each completed dataset, and fits 
# both proportional-odds and proportional-hazards Royston–Parmar models for the treatment intensification outcome.
...
for (i in imputation_range) {
  ...
  imp_i <- mice(mi_data, m = 1, maxit = 20, seed = 1100 + i, predictorMatrix=pred)
  
  # Extract the completed dataset
  data_i <- complete(imp_i, 1)
  
  # Save imputed dataset (optional)
  saveRDS(data_i, paste0("mi_na_intens_", i, ".rds"))
  
  # Fit Royston-Parmar model
  rp_model_i <- flexsurvspline(
    Surv(time_change_yrs, intens_event) ~ lastpre1oaddep_quant + 
  ...
  # Save model
  saveRDS(rp_model_i, paste0("mi_na_YEARrpm_intens_", i, ".rds"))
  
  # Clean up
  rm(imp_i, data_i, rp_model_i)
  gc()
}
...
for (i in 1:n_imputations) {
  model_i <- readRDS(paste0("mi_na_YEARrpm_intens_", i, ".rds"))
  rp_models[[i]] <- model_i
  coef_list[[i]] <- coef(model_i)
  vcov_list[[i]] <- vcov(model_i)
  bic_po_list[[i]] <- model_i$BIC
}
...
saveRDS(pooled, "na_rp_intens_YEARpooled.rds")
saveRDS(summary_pooled, "na_pooled_intens_YEARsummary.rds")

############################################
### Extract and exponentiate depression terms (ORs)
############################################

### Filter to depression-related terms (adjust the pattern to match your variable names)
depression_rows <- summary_pooled[grep("lastpre1oaddep_quantMDD", rownames(summary_pooled)), ]

### Add ORs and CIs on the exponentiated scale
...
### Save
saveRDS(depression_ORs, "na_pooled_depOR_intens_YEARmi.rds")
...
bic_ph_list <- list()
for (i in 1:n_imputations) {
  model_i <- readRDS(paste0("mi_na_YEARrpm_PH_intens_", i, ".rds"))
  rp_models[[i]] <- model_i
  coef_list[[i]] <- coef(model_i)
  vcov_list[[i]] <- vcov(model_i)
  bic_ph_list[[i]] <- model_i$BIC
}
...
### Save the final results
saveRDS(pooled, "na_rpPH_intens_YEARpooled.rds")
saveRDS(summary_pooled, "na_PH_pooled_intens_YEARsummary.rds")
...
### Save results
saveRDS(depression_PHs, "na_pooled_depHR_intens_YEARmi.rds")
...
write.csv(out, file = "intensbic_po_ph.csv")