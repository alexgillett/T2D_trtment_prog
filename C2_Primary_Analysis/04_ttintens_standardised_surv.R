############################################################################################
############################################################################################
### Standardised survival curve code: Treatment intensification
############################################################################################
############################################################################################
...
########################################
### NOTE: see 03_ttinsulin_standardised_surv.R for code to generate the IDs
...
message("Running surv ", i)
  ...
  saveRDS(standsurv_i, paste0("standsurv_mi_intens_", i, ".rds"))
...
survcurve_out <- list()
for(i in 1:n_imps){
    out_i <- readRDS(paste0("standsurv_mi_intens_", i, ".rds"))
...
saveRDS(pooled_surv, file="mi_intens_pooled_surv.rds")
...
standsurv_i <- standsurv(model_i, newdata = dat_i, t=times_r, type = "survival", 
  ...
  ### save survival data i
  saveRDS(standsurv_i, paste0("standsurvdiff_mi_intens_", i, ".rds"))
  rm(dat_i, model_i, standsurv_i)
...
for(i in 1:n_imps){
    out_i <- readRDS(paste0("standsurvdiff_mi_intens_", i, ".rds"))
...
saveRDS(pooled_surv_restricted, "mi_intens_pooled_survreduced.rds")
saveRDS(pooled_contrasts, "mi_intens_pooled_riskdiff.rds")
...
message("Running rmst ", i)
  ...
  rmst_i <- standsurv(model_i, newdata = dat_i, t=times_r, type = "rmst", 
                    ...
                    at = list(list(lastpre1oaddep_quant = "No MDD"),
                                                       ...
                                                       list(lastpre1oaddep_quant = "MDD <= 1.7yrs")))
  ### save survival data i
  saveRDS(rmst_i, paste0("rmst_mi_intens_", i, ".rds"))
...
for(i in 1:n_imps){
    out_i <- readRDS(paste0("rmst_mi_intens_", i, ".rds"))
    rmst_out[[i]] <- out_i
}
...
saveRDS(rmst_pooled, "mi_intens_pooled_rmst.rds")
saveRDS(rmst_diff_pooled, "mi_intens_pooled_rmstdiff.rds")